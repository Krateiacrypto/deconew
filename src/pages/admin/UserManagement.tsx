...
import { Users, Search, Filter, Download, Eye, CreditCard as Edit, Trash2, CheckCircle, XCircle, Shield, Mail, Phone, Plus, UserPlus, Building, Award, Calendar, Activity, AlertTriangle, User, Settings, Clock } from 'lucide-react';
...
export const UserManagement: React.FC = () => {
  ...
  /**
   * Load users using useAsyncData hook
   */
  const { loading: isLoading } = useAsyncData(
    async () => {
   ...
   */
  const { mutate: toggleStatus } = useAsyncMutation(
    async (userId: string) => {
      if (!hasPermission(user, 'users.edit')) {
        throw new Error('Kullanıcı düzenleme yetkiniz bulunmamaktadır!');
      }
    ...
    },
    {
      ...
      successMessage: (userId?: string) => {
        const targetUser = users.find(u => u.id === userId);
        return `Kullanıcı ${targetUser?.isActive ? 'deaktif' : 'aktif'} edildi!`;
      },
      autoLog: true,
    }
  );
   ...
   */
  const { mutate: removeUser } = useAsyncMutation(
    async (userId: string) => {
      if (!hasPermission(user, 'users.delete')) {
        throw new Error('Kullanıcı silme yetkiniz bulunmamaktadır!');
      }
   ...
   */
  const { mutate: updateKYCStatus } = useAsyncMutation(
    async (payload: { userId: string; status: 'approved' | 'rejected' }) => {
      await updateUser(payload.userId, { kycStatus: payload.status });
      return { success: true };
    },
    {
      showErrorToast: true,
      showSuccessToast: true,
      successMessage: (payload?: any) =>
        `KYC durumu ${payload?.status === 'approved' ? 'onaylandı' : 'reddedildi'}!`,
      autoLog: true,
    }
  );
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center space-x-2 bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isRefreshing ? (
