...
export const KYCManagement: React.FC = () => {
  ...
  const { applications, fetchKYCApplications, reviewKYCApplication } = useKYCStore();
  ...
  /**
   * Load KYC applications using useAsyncData hook
   */
  const { loading: isLoading } = useAsyncData(
    ...
    {
      showErrorToast: true,
      errorMessage: 'KYC başvuruları yüklenirken hata oluştu',
      autoLog: true,
    }
  );
  ...
  /**
   * Review KYC application mutation using useAsyncMutation hook
   */
  const { mutate: submitReview, loading: isReviewing } = useAsyncMutation(
    async (payload: { applicationId: string; status: 'approved' | 'rejected'; notes?: string }) => {
      if (!hasPermission(user, 'users.edit')) {
        throw new Error('KYC onaylama yetkiniz bulunmamaktadır!');
      }

      await reviewKYCApplication(payload.applicationId, payload.status, payload.notes);
      return { success: true };
    },
    {
      showErrorToast: true,
      ...
      successMessage: (payload?: any) =>
        `KYC başvurusu ${payload?.status === 'approved' ? 'onaylandı' : 'reddedildi'}`,
      autoLog: true,
    }
  );

  const handleReview = async (applicationId: string, status: 'approved' | 'rejected', notes?: string) => {
    await submitReview({ applicationId, status, notes });
  };
