...
export const getRoleDisplayName = (role: User['role']): string => {
  const roleNames: Record<User['role'], string> = {
    ...
    web_admin: 'Web Admin',
    advisor: 'Danışman',
    verifier: 'Doğrulama Kuruluşu',
    verification_org: 'Doğrulama Kuruluşu', // Legacy support
    ngo: 'STK',
    carbon_provider: 'Karbon Sağlayıcı',
    institutional_investor: 'Kurumsal Yatırımcı',
    pro_investor: 'Pro Yatırımcı',
    free_investor: 'Ücretsiz Yatırımcı',
    user: 'Kullanıcı'
  };
...
export const getRoleColor = (role: User['role']): string => {
  const roleColors: Record<User['role'], string> = {
    ...
    verification_org: 'bg-green-100 text-green-800', // Legacy support
