...
export const KYCForm: React.FC = () => {
  ...
  const { submitKYCApplication, uploadDocument } = useKYCStore();
  ...
  /**
   * KYC submission mutation using useAsyncMutation hook
   * Handles document uploads and application submission
   */
  const { mutate: submitKYCData, loading: isLoading } = useAsyncMutation(
    async () => {
      if (!user) {
    ...
    },
    {
      ...
      successMessage: 'KYC başvurunuz başarıyla gönderildi!',
      errorMessage: 'KYC başvurusu gönderilirken hata oluştu',
      autoLog: true,
      onSuccess: () => {
        setStep(4);
      },
