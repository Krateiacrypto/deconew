...
export const BlogComments: React.FC<BlogCommentsProps> = ({ postId }) => {
  ...
  /**
   * Load comments using useAsyncData hook
   */
  const { loading: isCommentsLoading } = useAsyncData(
    async () => {
   ...
   */
  const { mutate: submitComment, loading: isSubmitting } = useAsyncMutation(
    async () => {
      if (!newComment.trim()) {
        throw new Error('Lütfen bir yorum yazın');
      }

      const authorName = user?.name || guestInfo.name;
      const authorEmail = user?.email || guestInfo.email;

      if (!user && (!guestInfo.name || !guestInfo.email)) {
        throw new Error('Lütfen ad ve e-posta alanlarını doldurun');
      }
    ...
    },
    {
      ...
      successMessage: 'Yorumunuz eklendi!',
      autoLog: true,
      onSuccess: async () => {
        setNewComment('');
   ...
   */
  const { mutate: submitReply, loading: isReplySubmitting } = useAsyncMutation(
    async (parentId: string) => {
      if (!replyContent.trim()) {
        throw new Error('Lütfen bir yanıt yazın');
      }
      ...
      if (!user && (!guestInfo.name || !guestInfo.email)) {
        throw new Error('Yanıt yapmak için ad ve e-posta gerekli');
      }
    ...
    },
    {
      showErrorToast: true,
      showSuccessToast: true,
      successMessage: 'Yanıtınız eklendi!',
      autoLog: true,
      onSuccess: async () => {
        setReplyContent('');
        setReplyingTo(null);
   ...
   */
  const { mutate: likeCommentAction } = useAsyncMutation(
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-2 mt-2">
                            <button
                              ...
                              disabled={!replyContent.trim() || isReplySubmitting || (!user && (!guestInfo.name || !guestInfo.email))}
                              className="bg-emerald-600 text-white px-3 py-1 rounded text-sm hover:bg-emerald-700 disabled:opacity-50"
                            >
                              {isReplySubmitting ? 'Gönderiliyor...' : 'Yanıtla'}
                            </button>
                            <button
                              onClick={() => {
                                setReplyingTo(null);
